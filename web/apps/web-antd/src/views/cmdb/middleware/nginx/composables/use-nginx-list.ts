...
export interface NginxFilters {
  status?: 'online' | 'offline';
}

export function useNginxList() {
  const instances = ref<NginxInstance[]>([]);
  const total = ref(0);
  const page = ref(1);
  const pageSize = ref(20);
  const loading = ref(false);
  const error = ref<Error | null>(null);
  const filters = ref<NginxFilters>({});

  const selectedInstance = ref<NginxInstance | null>(null);
  ...
  const applyFilters = async (nextFilters: NginxFilters) => {
  ...
  const deleteInstance = async (instanceId: number) => {
    try {
      await deleteNginxInstanceApi(instanceId);
      message.success('删除成功');
      await fetchInstances();
    } catch (err) {
      message.error('删除 Nginx 实例失败');
    }
  };
  ...
  const getInstanceDetail = async (instanceId: number) => {
    ...
    try {
      const result = (await getNginxInstanceApi(instanceId)) as NginxInstance;
      selectedInstance.value = result ?? null;
    } catch (err) {
      message.error('获取 Nginx 详情失败');
    } finally {

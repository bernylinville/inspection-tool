...
type ElasticsearchAlert struct {
	...
	Level             pkgmodel.AlertLevel `json:"level"`               // 告警级别 (复用 alert.go 的 pkgmodel.AlertLevel)
	Message           string     `json:"message"`             // 告警消息
}

// NewElasticsearchAlert creates a new ElasticsearchAlert with given parameters.
func NewElasticsearchAlert(address, metricName string, currentValue float64, level pkgmodel.AlertLevel) *ElasticsearchAlert {
...
func (r *ElasticsearchInspectionResult) AddAlert(alert *ElasticsearchAlert) {
	...
	} else if alert.Level == pkgmodel.AlertLevelWarning && r.Status != ElasticsearchStatusCritical {
...
func NewElasticsearchAlertSummary(alerts []*ElasticsearchAlert) *ElasticsearchAlertSummary {
	...
	for _, alert := range alerts {
		...
		summary.TotalAlerts++
		switch alert.Level {
		case pkgmodel.AlertLevelWarning:
			summary.WarningCount++
		case pkgmodel.AlertLevelCritical:

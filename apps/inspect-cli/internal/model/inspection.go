...
func NewInspectionSummary(hosts []*HostResult) *InspectionSummary {
	...
	for _, host := range hosts {
		...
		summary.TotalHosts++
		switch host.Status {
		case pkgmodel.HostStatusNormal:
			summary.NormalHosts++
		case pkgmodel.HostStatusWarning:
			summary.WarningHosts++
		case pkgmodel.HostStatusCritical:
			summary.CriticalHosts++
		case pkgmodel.HostStatusFailed:
			summary.FailedHosts++
		}
	}
	return summary
...
type HostResult struct {
	...
	Status        pkgmodel.HostStatus `json:"status"` // 整体状态

	// 指标数据
	Metrics map[string]*pkgmodel.MetricValue `json:"metrics"` // 指标集合，key = 指标名称

	// 告警信息
	Alerts []*pkgmodel.Alert `json:"alerts,omitempty"` // 该主机的告警列表
...
func NewHostResult(meta *pkgmodel.HostMeta) *HostResult {
	if meta == nil {
		return &HostResult{
			Status:  pkgmodel.HostStatusFailed,
			Metrics: make(map[string]*pkgmodel.MetricValue),
		}
	}
	return &HostResult{
		...
		Metrics:       make(map[string]*pkgmodel.MetricValue),
		Alerts:        make([]*pkgmodel.Alert, 0),
	}
}

// SetMetric adds or updates a metric value for this host.
func (r *HostResult) SetMetric(value *pkgmodel.MetricValue) {
	if r.Metrics == nil {
		r.Metrics = make(map[string]*pkgmodel.MetricValue)
	}
	if value != nil {
...
func (r *HostResult) GetMetric(name string) *pkgmodel.MetricValue {
	if r.Metrics == nil {
		return nil
	}
...
func (r *HostResult) AddAlert(alert *pkgmodel.Alert) {
	if alert == nil {
		return
	}
	r.Alerts = append(r.Alerts, alert)
	// Update host status to the most severe alert level
	if alert.Level == pkgmodel.AlertLevelCritical {
		r.Status = pkgmodel.HostStatusCritical
	} else if alert.Level == pkgmodel.AlertLevelWarning && r.Status != pkgmodel.HostStatusCritical {
		r.Status = pkgmodel.HostStatusWarning
	}
}
...
type InspectionResult struct {
	...
	// 告警汇总
	Alerts       []*pkgmodel.Alert      `json:"alerts"`        // 所有告警列表
	AlertSummary *pkgmodel.AlertSummary `json:"alert_summary"` // 告警摘要统计
...
func (r *InspectionResult) Finalize(endTime time.Time) {
	...
	r.AlertSummary = pkgmodel.NewAlertSummary(r.Alerts)
}
...
func (r *InspectionResult) GetCriticalHosts() []*HostResult {
	...
	for _, host := range r.Hosts {
		if host != nil && host.Status == pkgmodel.HostStatusCritical {
...
func (r *InspectionResult) GetWarningHosts() []*HostResult {
	...
	for _, host := range r.Hosts {
		if host != nil && host.Status == pkgmodel.HostStatusWarning {
...
func (r *InspectionResult) GetFailedHosts() []*HostResult {
	...
	for _, host := range r.Hosts {
		if host != nil && host.Status == pkgmodel.HostStatusFailed {

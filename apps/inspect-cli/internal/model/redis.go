...
func NewRedisAlert(address, metricName string, currentValue float64, level pkgmodel.AlertLevel) *RedisAlert {
	return &RedisAlert{
		Address:      address,
...
func (r *RedisInspectionResult) AddAlert(alert *RedisAlert) {
	...
	} else if alert.Level == pkgmodel.AlertLevelWarning && r.Status != RedisStatusCritical {

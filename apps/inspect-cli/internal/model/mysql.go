...
func NewMySQLAlert(address, metricName string, currentValue float64, level pkgmodel.AlertLevel) *MySQLAlert {
	return &MySQLAlert{
		Address:      address,
...
func (r *MySQLInspectionResult) AddAlert(alert *MySQLAlert) {
	...
	} else if alert.Level == pkgmodel.AlertLevelWarning && r.Status != MySQLStatusCritical {
